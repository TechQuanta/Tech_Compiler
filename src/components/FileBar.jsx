...
const FileBar = ({
  ...
  const getLanguageVersions = (langValue) => {
    const lang = Object.values(languageOptions).find(opt => opt.value === langValue);
  ...
  useEffect(() => {
    const currentLangOption = Object.values(languageOptions).find(opt => opt.value === language);
  ...
  return (
    <nav className="fixed top-[20px] md:top-[40px] left-0 w-full h-[50px] md:h-[50px] flex items-center justify-between px-2 md:px-4 z-40 shadow-xl border-b
        ...
        <AnimatePresence mode="popLayout">
          {openFiles.map((file) => (
            <motion.button
              ...
              exit={{ opacity: 0, x: 50, transition: { duration: 0.2, ease: "easeOut" } }}
      ...
      <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
        ...
        <button
          onClick={onRunCode}
          disabled={isLoading}
          className={`px-3 py-1 md:px-5 md:py-2 rounded-md font-semibold text-xs md:text-base transition-all duration-200 focus:outline-none focus:ring-2 shadow-md hover:shadow-lg flex items-center gap-1
            ${isLoading ? "opacity-60 cursor-not-allowed" : ""}
            bg-light-accent-green text-light-text-primary hover:bg-light-accent-green/80 focus:ring-light-accent-green focus:ring-opacity-70 hover:shadow-light-accent-green/40
        ...
        <div className="relative inline-block">
          <select
            ...
            disabled={!getLanguageVersions(language) || getLanguageVersions(language).length === 0}
            className="rounded-md pl-2 pr-6 py-1 md:px-3 md:py-2 text-xs md:text-sm appearance-none focus:outline-none focus:ring-2 cursor-pointer shadow-sm
                         bg-light-input-bg text-light-text-primary focus:ring-light-accent-blue focus:ring-opacity-70
                         dark:bg-dark-input-bg dark:text-dark-text-primary dark:focus:ring-dark-accent-blue dark:focus:ring-opacity-70"
          >
            ...
            {(!getLanguageVersions(language) || getLanguageVersions(language).length === 0) && (
