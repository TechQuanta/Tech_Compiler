...
const CodeEditor = () => {
  ...
  const [output, setOutput] = useState({ stdout: '', stderr: '', compile_output: '' });
  ...
  const onFileSelect = useCallback((id) => {
  ...
  const onFileClose = useCallback((idToClose) => {
    setOpenFiles(prevFiles => {
      ...
      if (remainingFiles.length === 0) {
        ...
        const newFile = { id: newFileId, name: `untitled.js`, content: CODE_SNIPPETS['javascript'], language: 'javascript' };
  ...
  const onNewFile = useCallback(() => {
    ...
    const newFile = { id: newFileId, name: newFileName, content: '', language: 'javascript' };
  ...
  const runCode = useCallback(async () => {
    ...
    try {
      ...
      setOutput({ stdout: '', stderr: `Execution Error: ${error.message || "Unable to run code"}`, compile_output: '' });
      setIsError(true);
      setShowOutput(true);
    } finally {
      setIsLoading(false);
  ...
  return (
    <div
      className={`w-full h-screen flex flex-col overflow-hidden
                  ${theme === 'dark' ? 'bg-dark-background-primary' : 'bg-light-background-primary'}`} // Use theme background
      ref={containerRef}
    >
      <TopBar />
